...
export function reloadImportDocuments(): void {
  const custumIncludePatternStr = workspace
    .getConfiguration('ecb2')
    .get<string>('custumIncludePattern')
  const customIncludePatternStr = workspace
    .getConfiguration('ecb2')
    .get<string>('customIncludePattern')

  customIncludeDirs = workspace
    .getConfiguration('ecb2')
    .get<string[]>('customIncludeDirs')

  if (
    custumIncludePatternStr !== customIncludePatternStr &&
    custumIncludePatternStr !== ''
  )
    customIncludePattern = new RegExp(custumIncludePatternStr, 'g')
  else customIncludePattern = new RegExp(customIncludePatternStr, 'g')

  const SourceImportFiles = workspace
    .getConfiguration('ecb2')
    .get<string[]>('includes')
  for (const key of Includes.keys()) {
    if (key.startsWith('Include')) Includes.delete(key)
  }
...
export function getImportsWithLocal(
  doc: TextDocument
): [string, IncludeFile][] {
...
export function getImportsWithLocal2(
  doc: TextDocument
): [string, IncludeFile][] {
  const localIncludes = [...Includes]
  const processedMatches = Array<string>()
