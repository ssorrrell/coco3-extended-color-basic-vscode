...
function getCurrentFunction(code: string) {
  ...
  let index: number
  if (parenSplit.length === 1) index = 0
  else index = parenSplit.length - 2
...
function getSignatures(
  text: string,
  docComment: string
): Map<string, SignatureInformation[]> {
...
function provideSignatureHelp(
  ...
  else sighelp.activeSignature = 0
  sighelp.activeParameter = caller.commas

  let sig: SignatureInformation[] | undefined
  if (
    (sig = getSignatures(doc.getText(), 'Local').get(caller.func)) !== undefined
  ) {
    sighelp.signatures.push(
      ...sig.filter(
        (sig2: SignatureInformation) => sig2.parameters.length >= caller.commas
      )
    )
  }
  ...
  for (const item of getImportsWithLocal(doc)) {
    if (
      (sig = getSignatures(item[1].Content, item[0]).get(caller.func)) !==
      undefined
    ) {
      sighelp.signatures.push(
        ...sig.filter(
          (sig2: SignatureInformation) =>
            sig2.parameters.length >= caller.commas
        )
      )
    }
  }
