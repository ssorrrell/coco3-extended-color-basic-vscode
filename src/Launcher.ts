...
export class VbsDebugSession extends LoggingDebugSession {
  ...
  protected async launchRequest(
    ...
    this._runner.stderr.on('data', data => {
      ...
      if (match) {
        ...
        const diag = new vscode.Diagnostic(
          new vscode.Range(line, char, line, char),
          match[3],
    ...
    this._runner.on('exit', code => {
      this.sendEvent(
        new OutputEvent(
          `Process [${this._runner.pid}] exited with code ${code} (&H${code
            .toString(16)
            .toUpperCase()})`
        )
      )
...
const launchConfigProvider = vscode.debug.registerDebugConfigurationProvider(
  'ecb2',
  new DebugConfigurationProvider()
)
const inlineDebugAdapterFactory =
